...
pub fn print(netlist: *const Netlist, placement: *Placement, allocator: std.mem.Allocator) void {
...
fn cost(netlist: *const Netlist, the_placement: *const Placement, annealing_config: AnnealingConfig, gpa: std.mem.Allocator) f32 {
    return costWireLength(netlist, the_placement, annealing_config) + computeCongestion(netlist, the_placement, annealing_config, false, gpa); // + costOverlap(the_graph, the_placement);
}
...
// congestion!
fn computeCongestion(netlist: *const Netlist, the_placement: *const Placement, annealing_config: AnnealingConfig, printthisgrid: bool, gpa: std.mem.Allocator) f32 {
    ...
    for (netlist.nets) |*net| {
        add_rudy_fast(netlist, the_placement, annealing_config, net, &grid);
    }
    var sum: f32 = 0;
    for (0..widthtemp) |i| {
        for (0..heighttemp) |j| {
...
fn add_rudy_fast(netlist: *const Netlist, the_placement: *const Placement, annealing_config: AnnealingConfig, net: *const Net, grid: *Density) void {
    const positions = getPortAbsolutePositions(netlist, the_placement, net, use_accurate_input_pos, annealing_config.chip_height_coordinate).?;
...
pub fn placement_annealing(gpa: std.mem.Allocator, netlist: *const Netlist, seed: u32, annealing_config: AnnealingConfig) ?*Placement {
    ...
    while (temperature > annealing_config.minimum_temperature) {
        ...
        const debug_congestion_cost = computeCongestion(netlist, current_placement, annealing_config, true, gpa);
        std.log.info(
