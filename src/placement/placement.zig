...
pub fn print(netlist: *const Netlist, placement: *Placement, allocator: std.mem.Allocator, printOccupancy: bool) void {
...
fn place(the_placement: *Placement, id: Id, new_x: postype, new_y: postype, orientation: Orientation, node_padding: u8) !void {
    ...
    try the_placement.locations.put(id, .{ .orientation = orientation, .x = new_x, .y = new_y });
}
...
fn moveInputOrOutputAxis(netlist: *const Netlist, the_placement: *Placement, window_size: u32, random: std.Random, min_spacing: u8, useInput: bool, node_padding: u8) glib.NodeId {
    ...
    while (it2.next()) |entry| {
        ...
        if (result != empty) {
            print(netlist, the_placement, tempgpa.?, true);
...
pub fn placement_annealing(gpa: std.mem.Allocator, netlist: *const Netlist, seed: u32, annealing_config: AnnealingConfig) ?*Placement {
    ...
    print(netlist, current_placement, gpa, false);
