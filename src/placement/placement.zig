...
var tempgpa: ?std.mem.Allocator = null;
...
pub const AnnealingConfig = struct {
    ...
    fix_inoutputs: bool = true,
    initial_input_y: postype = 0,
    initial_output_y: postype = 100,
    // for convenience:
...
pub const Position = struct { x: postype, y: postype, orientation: Orientation, mirrorred: bool };
...
pub fn printOccupancyGrid(self: *const Placement, string: *std.io.Writer.Allocating) void {
    ...
    while (y > 0) {
        ...
        for (0..max_chipsize) |x| {
            const char = self.occupancy_grid[x][y];
            if (char == empty) {
                string.writer.print(" {c}", .{'o'}) catch {};
            } else {
...
pub fn printCongestionGrid(grid: Density, string: *std.io.Writer.Allocating) void {
    ...
    for (0..grid.cell_count_y) |i_rev| {
        ...
        for (0..grid.cell_count_x) |j_rev| {
            ...
            // print colored block
            string.writer.print("\x1b[48;5;{d}m  \x1b[0m", .{color}) catch {};
        }
        string.writer.print("\n", .{}) catch {};
    }
}

fn heatColor(norm: f32) u8 {
    // clamp
    const t = if (norm < 0) 0 else if (norm > 1) 1 else norm;

    // piecewise gradient: blue → green → yellow → red
    if (t < 0.33) {
        // blue → green
        return @intFromFloat(21 + t / 0.33 * (46 - 21));
    } else if (t < 0.66) {
        // green → yellow
        return @intFromFloat(46 + (t - 0.33) / 0.33 * (226 - 46));
    } else {
        // yellow → red
        return @intFromFloat(226 + (t - 0.66) / 0.34 * (196 - 226));
    }
}
// end of generated by chat.

pub fn print(placement: *Placement, allocator: std.mem.Allocator, printOccupancy: bool) void {
    ...
    try renderAscii(placement, &string);
    if (printOccupancy) {
...
fn getPortAbsolutePositions(the_placement: *const Placement, edge: *const Net, chip_height_coordinate: postype) ?[2]model.Pos {
    ...
    const input_instance_mirrorred = the_placement.locations.get(to_node.instance).?.mirrorred;
...
fn initialPlacement(gpa: std.mem.Allocator, netlist: *const Netlist, annealing_config: AnnealingConfig) *Placement {
    errdefer @panic("Ran out of memory lol");
    const placement = try gpa.create(Placement);
    placement.init(gpa);
    ...
    for (netlist.instances, 0..) |*instance, i| {
        ...
        if (annealing_config.fix_inoutputs and netlist.numInputPorts(i) == 0) {
        ...
        if (annealing_config.fix_inoutputs and netlist.numOutputPorts(i) == 0) {
            std.debug.assert(placement.output_nodes.contains(i) == false);
            std.debug.assert(placement.input_nodes.contains(i) == false);
    ...
    while (it.next()) |entry| {
        ...
        try place(placement, entry.key_ptr.*, x, y, .North, annealing_config.node_padding, false);
        count += 1;
    ...
    for (0..netlist.instances.len) |node_id| {
        ...
        try place(placement, node_id, x, y, .North, annealing_config.node_padding, false);
        count += 1;
        std.debug.assert(placement.locations.count() == count);
        x = x + spacing;
...
fn cost(netlist: *const Netlist, the_placement: *const Placement, annealing_config: AnnealingConfig, gpa: std.mem.Allocator) f32 {
    return costWireLength(netlist, the_placement, annealing_config) + computeCongestionRUDY(netlist, the_placement, annealing_config, false, gpa); // + costOverlap(the_graph, the_placement);
}
...
fn costWireLength(netlist: *const Netlist, the_placement: *const Placement, annealing_config: AnnealingConfig) f32 {
    var sum: f32 = 0;
    for (netlist.nets) |*net| {
        const netcost = HPWL(netlist, the_placement, annealing_config, net);
        sum = sum + netcost;
    }
...
fn place(the_placement: *Placement, id: Id, new_x: postype, new_y: postype, orientation: Orientation, node_padding: u8, mirrorred: bool) !void {
    ...
    for (new_x - node_padding..new_x + rect.w) |x| {
        for (new_y - node_padding..new_y + rect.h) |y| {
            const assertion: bool = the_placement.occupancy_grid[x][y] == empty or the_placement.occupancy_grid[x][y] == id;
            if (!assertion) {
                print(the_placement, tempgpa.?, true);
                std.debug.print("attempted to place {d} (size:w:{d},h:{d}) at ({d},{d}) but at ({d},{d}) is {d}\n", .{ id, rect.w, rect.h, new_x, new_y, x, y, the_placement.occupancy_grid[x][y] });
            }
    ...
    try the_placement.locations.put(id, .{ .orientation = orientation, .x = new_x, .y = new_y, .mirrorred = mirrorred });
}

// clears a cells occupation in the occupancy grid
fn clearOccupancy(the_placement: *Placement, id: Id, node_padding: u8) void {
    const variant = the_placement.variants.get(id).?;
    const rect = variant.model.brect();
    const pos = the_placement.locations.get(id).?;
    for (pos.x - node_padding..pos.x + rect.w) |x| {
        for (pos.y - node_padding..pos.y + rect.h) |y| {
...
// checks for collision at a new position
fn checkCollision(the_placement: *const Placement, node_id: glib.NodeId, new_x: postype, new_y: postype, node_padding: u8) !glib.NodeId {
    const variant = the_placement.variants.get(node_id).?;
    const rect = variant.model.brect();
...
// checks for collision at a new position, while ignoring another node's id due to an expected swap operation
fn checkCollisionSwap(the_placement: *const Placement, node_id: glib.NodeId, new_x: postype, new_y: postype, node_padding: u8, to_ignore: glib.NodeId) !glib.NodeId {
    const variant = the_placement.variants.get(node_id).?;
    const rect = variant.model.brect();

    for (new_x - node_padding..new_x + rect.w) |x| {
        for (new_y - node_padding..new_y + rect.h) |y| {
            if (the_placement.occupancy_grid[x][y] != empty and the_placement.occupancy_grid[x][y] != to_ignore) {
                return the_placement.occupancy_grid[x][y];
            }
        }
    }
...
fn tryMove(the_placement: *Placement, id: Id, last_pos: *const Position, new_x: postype, new_y: postype, node_padding: u8) !glib.NodeId {
    ...
    const collision_result = try checkCollision(the_placement, id, new_x, new_y, node_padding);
    if (collision_result != empty) return collision_result;

    // No collision, so move rectangle
    clearOccupancy(the_placement, id, node_padding);

    try place(the_placement, id, new_x, new_y, last_pos.orientation, node_padding, last_pos.mirrorred);
...
// mirrors a cell by swapping the nets connected to the input ports.
// For more cells that are not trivially mirrorrable this way, the entire schematic must be mirrorred!
fn mirrorCell(netlist: *const Netlist, the_placement: *Placement, to_perturb: glib.NodeId) void {
    errdefer @panic("an error! oh no!");
    _ = netlist;
    const currentpos = the_placement.locations.get(to_perturb).?;

    try the_placement.locations.put(to_perturb, .{ .orientation = currentpos.orientation, .x = currentpos.x, .y = currentpos.y, .mirrorred = !currentpos.mirrorred });
}
...
fn swap(netlist: *const Netlist, the_placement: *Placement, node_a_id: glib.NodeId, node_b_id: glib.NodeId, node_padding: u8) bool {
    ...
    const coll_res_b = try checkCollisionSwap(the_placement, node_b_id, unsigned_new_x_b, unsigned_new_y_b, node_padding, node_a_id);
    ...
    const result1 = try checkCollision(the_placement, node_a_id, unsigned_new_x_a, unsigned_new_y_a, node_padding);
    const result2 = try checkCollision(the_placement, node_b_id, unsigned_new_x_b, unsigned_new_y_b, node_padding);
    if (result1 != empty or result2 != empty) {
        ...
        for (pos_b.x - node_padding..pos_b.x + rect_b.w) |x| {
            for (pos_b.y - node_padding..pos_a.y + rect_b.h) |y| {
                std.debug.print("checking b: ({d},{d}):{d} \n", .{ x, y, the_placement.occupancy_grid[x][y] });
                std.debug.assert(the_placement.occupancy_grid[x][y] == empty);
            }
        }
        std.debug.print("we shouldnt get here\n", .{});
    }
    std.debug.assert(result1 == empty);
    std.debug.assert(result2 == empty);

    // set a to b position
    try place(the_placement, node_a_id, unsigned_new_x_a, unsigned_new_y_a, pos_a.orientation, node_padding, pos_a.mirrorred);

    // lets do it again here lol
    const result3 = try checkCollision(the_placement, node_a_id, unsigned_new_x_a, unsigned_new_y_a, node_padding);
    ...
    std.debug.assert(result3 == empty);
    std.debug.assert(result4 == empty);

    // set b to a position
    try place(the_placement, node_b_id, unsigned_new_x_b, unsigned_new_y_b, pos_b.orientation, node_padding, pos_b.mirrorred);
...
fn moveInputOrOutputAxis(netlist: *const Netlist, the_placement: *Placement, window_size: u32, random: std.Random, min_spacing: u8, useInput: bool, node_padding: u8) glib.NodeId {
    ...
    while (it1.next()) |entry| {
        ...
        const collision_result = try checkCollision(the_placement, id, pos.x, new_y, node_padding);
        if (collision_result != empty) return collision_result;
    }
    ...
    while (it2.next()) |entry| {
        const id: glib.NodeId = entry.key_ptr.*;
        const variant = the_placement.variants.get(id).?; // debug
        const rect = variant.model.brect(); // debug
        const pos = the_placement.locations.getPtr(id).?;
        const collision_result = try checkCollision(the_placement, id, pos.x, new_y, node_padding); // debug
        const result = try tryMove(the_placement, id, pos, pos.x, new_y, node_padding);

        // debug:
        if (result != empty) {
            print(the_placement, tempgpa.?, true);
            std.debug.print("attempted to move {d} from: ({d},{d}) to ({d},{d})\n", .{ id, pos.x, pos.y, (pos.x), (new_y) });
            const posc = the_placement.locations.getPtr(collision_result).?;
            const variantc = the_placement.variants.get(collision_result).?;
            const rectc = variantc.model.brect();
            std.debug.print("collided with {d} at: ({d},{d}), size: (w:{d},h:{d})\n", .{ collision_result, posc.x, posc.y, rectc.w, rectc.h });

            for (pos.x - node_padding..pos.x + rect.w) |x| {
                for (new_y - node_padding..new_y + rect.h) |y| {
                    std.debug.print("checked: ({d},{d}):{d} \n", .{ x, y, the_placement.occupancy_grid[x][y] });
                    std.debug.assert(the_placement.occupancy_grid[x][y] == empty);
                }
            }
            std.debug.print("we shouldnt get here\n", .{});
...
fn perturb(netlist: *const Netlist, the_placement: *Placement, random: std.Random, current_window_size: f32, annealing_config: AnnealingConfig) void {
    errdefer @panic("Ran out of memory lol");
    for (0..annealing_config.perturbations_amount) |_| {
        ...
        if (isOutput(the_placement, to_perturb)) {
            ...
            if (random.intRangeLessThan(usize, 0, 10) != 1) {
                _ = randomMove(netlist, the_placement, to_perturb, @intFromFloat(@ceil(current_window_size)), random, annealing_config.grid_size, the_placement.output_y, annealing_config.node_padding);
            } else {
                _ = moveInputOrOutputAxis(netlist, the_placement, @intFromFloat(@ceil(current_window_size)), random, annealing_config.grid_size, false, annealing_config.node_padding);
            }
        } else if (isInput(the_placement, to_perturb)) {
            // perform moveOutput 1 out of 10 times
            if (random.intRangeLessThan(usize, 0, 10) != 1) {
                _ = randomMove(netlist, the_placement, to_perturb, @intFromFloat(@ceil(current_window_size)), random, annealing_config.grid_size, the_placement.input_y, annealing_config.node_padding);
            } else {
                _ = moveInputOrOutputAxis(netlist, the_placement, @intFromFloat(@ceil(current_window_size)), random, annealing_config.grid_size, true, annealing_config.node_padding);
            }
        } else {
            const resultmove = randomMove(netlist, the_placement, to_perturb, @intFromFloat(@ceil(current_window_size)), random, annealing_config.grid_size, null, annealing_config.node_padding); // use grid size as the minimum y position, so it stays nicely alligned if we changed it in the future
            // break if succesfully perturbed
            if (resultmove == empty) {
                return;
            }
            // break if succesfully perturbed
            const resultswap = swap(netlist, the_placement, to_perturb, resultmove, annealing_config.node_padding);
            if (resultswap) {
                return;
            }
            const resultmirror = mirrorCell(netlist, the_placement, to_perturb);
            _ = resultmirror;
        }
    }
}
...
pub fn placement_annealing(gpa: std.mem.Allocator, netlist: *const Netlist, seed: u32, annealing_config: AnnealingConfig) ?*Placement {
    errdefer @panic("Skill issue");

    tempgpa = gpa;

    var current_placement = initialPlacement(gpa, netlist, annealing_config);
    defer current_placement.deinit(gpa);
    print(current_placement, gpa, false);
    var temperature = annealing_config.initial_temperature;
    var current_window_size = annealing_config.initial_window_size;

    var best_placement = try current_placement.clone(gpa);
    ...
    var lastCost: f32 = cost(netlist, current_placement, annealing_config, gpa);
    while (temperature > annealing_config.minimum_temperature) {
        for (0..actual_moves_per_temperature) |i| {
            var new_placement = try current_placement.clone(gpa);

            perturb(netlist, new_placement, rand, current_window_size, annealing_config);
            const current_cost = lastCost;
            const new_cost = cost(netlist, new_placement, annealing_config, gpa);
            const cost_diff = new_cost - current_cost;
            if (cost_diff < 0) {
                current_placement.deinit(gpa);
            ...
            if (new_cost < best_cost) {
                best_cost = new_cost;
                best_placement.deinit(gpa);
                best_placement = try current_placement.clone(gpa);
            }
        } // exit loop if in equilibrium at this temperature

        if (annealing_config.print_results) {
            print(current_placement, gpa, false);
